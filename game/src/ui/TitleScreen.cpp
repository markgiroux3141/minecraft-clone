...
TitleScreen::Action TitleScreen::Draw(vox::UiRenderer& ui, glm::vec2 screen, glm::vec2 mouse,
    ...
    for (size_t i = 0; i < listed; ++i) {
        if (UiButton(ui, s, {x, y}, buttonSize, worlds[i], mouse, clicked)) {
    ...
    if (UiButton(ui, s, {x, y}, buttonSize, "New World", mouse, clicked)) {
        action.type = Action::Type::NewWorld;
    }
    y += buttonSize.y + gap;
    if (UiButton(ui, s, {x, y}, buttonSize, "Quit", mouse, clicked)) {
